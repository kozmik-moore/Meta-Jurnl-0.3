...
class AttributesFrame(Frame):
    def __init__(self, reader: ReaderModule, **kwargs):
        ...
        self.bind_class('JournalWidget', '<<Selected Id>>', self.set_buttons)
    ...
    def set_buttons(self, *args):
        self.attachments_btn.state(['!disabled' if self.reader.entry_attachments else 'disabled'])
    ...
    def attachments_popup(self):
        ...
        e = self.reader.id_
        d = Label(master=t, text=self.reader.get_date(e).strftime('%a, %b %d, %Y %H:%M'))
    ...
    def children_popup(self):
        ...
        for child in c:
            button = Button(master=f, text=self.reader.get_date(child).strftime('%a, %b %d, %Y %H:%M'),
